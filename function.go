...
func (f MaxFloat) Apply(event []Event) []Event {
	max := event[0].Float64Value(f.Name)
	for _, e := range event {
		max = math.Max(max, e.Float64Value(f.Name))
	}

	for _, e := range event {
		e.Record["max("+f.Name+")"] = float32(max)
	}
...
func (f MinFloat) Apply(event []Event) []Event {
	min := event[0].Float64Value(f.Name)
	for _, e := range event {
		min = math.Min(min, e.Float64Value(f.Name))
	}

	for _, e := range event {
		e.Record["min("+f.Name+")"] = float32(min)
	}
...
func (f MedianInt) Apply(event []Event) []Event {
	...
	center := len(values) / 2
	median := float32(values[center])
	if len(values)%2 == 0 {
		median = float32(values[center-1]+values[center]) / float32(2)
	}
...
func (f MedianFloat) Apply(event []Event) []Event {
	values := []float32{}
	for _, e := range event {
		values = append(values, e.Float32Value(f.Name))
	}
